...
jQuery(document).ready(function() {
	...
	jQuery('.responsive-menu-toggles').on('click', function (e) {
		e.preventDefault();
		var el = jQuery(this)
	...
	/* ---------------------------------------------------------------------------
		...
		function sideSlide(){
			...
			$( '.responsive-menu-toggles' ).off( 'click' );
			
			$( '.responsive-menu-toggles' ).on( 'click', function(e){
				e.preventDefault();
				button();			
			});
	...
	function onePageMenu() {
		if (jQuery('body').hasClass('one-page')) {
			...
			jQuery('#menu a[data-hash]').on( 'click',function(e) {
				...
				// FIX | responsive 
				var responsive = jQuery('.responsive-menu-toggles');
		...
		function onePageMenu(){
			if( $('body').hasClass('one-page') ){
				...
				$(document).on('click','#menu a[data-hash]', function (e) {
					...
					if( $(window).width() < 768 ){
						$('.responsive-menu-toggles').removeClass('active');
						$('#Top_bar #menu').hide();
					}
...
function mfnSliderOfferThumb_Pager(nr) {
	...
	var imgStyle = jQuery(this).closest('.offer_thumb').find('.offer_thumb_li.id_' + nr + ' .thumbnail img').attr('data-img-style');
	var aStyle = jQuery(this).closest('.offer_thumb').find('.offer_thumb_li.id_' + nr + ' .thumbnail img').attr('data-a-style');
	var title = jQuery(this).closest('.offer_thumb').find('.offer_thumb_li.id_' + nr + ' .thumbnail img').attr('data-title');
	return '<a style="'+aStyle+'" href="#' + nr + '"><img style="'+imgStyle+'" src="' + thumb + '" alt="' + nr + '" /><text>'+title+'</text></a>';
	// return '<a style="'+aStyle+'" href="#' + nr + '"><img style="'+imgStyle+'" src="' + thumb + '" alt="' + nr + '" /></a>';
}
...
function mfnSliderOfferThumb() {
	...
	function setOfferThumbHeight() {
        const paginationElement = document.getElementById('slider_pagination');
        const offerThumbElement = document.getElementById('offer_thumb');
        
        if (paginationElement && offerThumbElement) {
          // 检查窗口宽度
          if (window.innerWidth >= 768) {
            const paginationHeight = paginationElement.offsetHeight;
            offerThumbElement.style.minHeight = paginationHeight + 'px';
          } else {
            // 在小屏幕下移除高度设置
            offerThumbElement.style.height = 'auto';
          }
        }
      }
      // 页面加载完成后执行
      ...
      // 窗口大小改变时重新计算
      window.addEventListener('resize', setOfferThumbHeight);
}
// --- Blog ------------------------------------------------------------------	
